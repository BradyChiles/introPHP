...
//Model-Controller Area.  The PHP processing code goes in this area. 
	...
	{
		if($key == 'extraInterests'){
			foreach($_POST['extraInterests'] as $value){
				$tableBody .= "<tr>";				
				$tableBody .= "<td>extraInterests</td>";		
	...
	if(isset($_POST["studentStatus"])){
		$inStudentStatus = $_POST["studentStatus"];
	}
	$inExtraInterests = array();
	if(isset($_POST['extraInterests'])){
		foreach($_POST['extraInterests'] as $value){
			array_push($inExtraInterests,$value);
		}
	}
	$inExpectedGraduation = $_POST["expectedGraduation"];
...
<p>Student Status: <?php if(isset($inStudentStatus)){echo $inStudentStatus;} ?></p>
<p>Student Interests: 

		<?php
			if(isset($inExtraInterests)){
				$counter = 0;
				foreach($inExtraInterests as $value){
					if(!$counter == 0){
						echo ", " . $value;
					}else{
						echo $value;
...
<p>Expected Graduation: <?php echo $inExpectedGraduation; ?></p>
